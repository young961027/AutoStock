...
def dbgout(message):
    ...
    slack.chat.post_message('#stockbot', strbuf)
...
def get_movingaverage(code, window):
    ...
    try:
        ...
        dbgout(' >> 종목명 : ' + str(window) + '// 이동평균가격 : ' + str(ex))
        return None
    ...
    try:
        ...
        if current_price > target_price and current_price > ma5_price \
            ...
            if bought_qty > 0:
                ...
                dbgout(" >> 구매 종목 : " + str(stock_name) + ' : ' + str(code) +
                       " -> " + str(bought_qty) + "주 구매완료!" + "`")
    except Exception as ex:
